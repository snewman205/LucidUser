...
{
	global $mysqli,$db_table_prefix,$loggedInUser,$remember_me_length;
	...
	if(isset($loggedInUser->remember_me)) {
		...
		if($loggedInUser->remember_me == 0) {
			...
			if(isset($_COOKIE[$name])) {
				...
				$stmt->bind_param("s", $loggedInUser->remember_me_sessiid);
				$stmt->execute();
				$stmt->close();
...
function updateSessionObj() {
	...
	$stmt->bind_param("ss", $newObj, $loggedInUser->remember_me_sessiid);
	$stmt->execute();
	$stmt->close();

}

// Determine amount of time to persist login
